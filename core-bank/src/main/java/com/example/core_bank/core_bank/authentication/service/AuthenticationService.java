...
public class AuthenticationService {
    ...
    public void saveNumber(String email, String number) {
        storage.put(email, new NumberEntry(number, LocalDateTime.now().plusMinutes(10)));
    }

    public boolean getNumber(String email, String pinNumber) {
        NumberEntry entry = storage.remove(email);
        if (entry != null && entry.isValid()) {
            return entry.getNumber().equals(pinNumber);
        }
        return false;
    }
    ...
    public boolean checkEmailAndPinNumber(ReqAuthentication reqAuthentication) {
        Authentication authentication =
                authenticationRepository.findByEmail(reqAuthentication.getEmail())
    ...
    public boolean checkEmailNumber(ReqAuthentication reqAuthentication) {
        return getNumber(reqAuthentication.getEmail(), reqAuthentication.getPinNumber());
    }

    public String sendEmailPinNumber(String email) {
