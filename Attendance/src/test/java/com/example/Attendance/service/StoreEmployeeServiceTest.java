...
public class StoreEmployeeServiceTest {
    ...
    void testFindStoreEmployeeByEmailAndStoreId_ValidLocation() {
        ...
        when(cryptography.decrypt(encryptedEmail)).thenReturn(email);
        // When: findStoreEmployeeByEmailAndStoreId 호출
        when(storeEmployeeRepository.findByEmailAndStoreId(email, 1))
                .thenReturn(Optional.of(storeEmployee));
        StoreEmployee result = storeEmployeeService.findStoreEmployeeByEmailAndStoreId(1, request,encryptedEmail);
    ...
    void testFindStoreEmployeeByEmailAndStoreId_InvalidLocation() {
        ...
        CustomException thrown = assertThrows(CustomException.class, () -> {
            storeEmployeeService.findStoreEmployeeByEmailAndStoreId(1, request,encryptedEmail);
        });
