...
public class PasswordEmailController {
    ...
    public ResponseEntity<String> ResetPassword(@RequestBody @Valid EmailRequest emailRequest) {
        ...
        // 이메일과 이름 일치하는지 확인
        President president = emailService.validateEmailAndName(emailRequest.getEmail(), emailRequest.getName());

        // 임시 비밀번호 생성하고 이메일 전송
        String temporaryPassword = emailService.joinEmail(emailRequest.getEmail());

        // 임시 비밀번호로 DB 업데이트
        emailService.updatePassword(temporaryPassword, president);
